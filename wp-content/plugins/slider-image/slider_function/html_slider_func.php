...
function hugeit_slider_print_html_nav( $count_items, $page_number, $serch_fields = "" ) {
	?>
	<script type="text/javascript">
		...
		function submit_href(x, y) {
			var items_county =<?php if ( $count_items ) {
				echo $count_items % 20 ? ( $count_items - $count_items % 20 ) / 20 + 1 : ( $count_items - $count_items % 20 ) / 20;
			} else {
				echo 1;
			}?>;
	...
	<?php if ( isset( $_POST['page_number'] ) ) {
		$pagenumber = absint( $_POST['page_number'] );
		?>
	...
	if ( isset( $_POST["serch_or_not"] ) ) {
		$serach_or_not = sanitize_text_field( $_POST["serch_or_not"] );
		?>
